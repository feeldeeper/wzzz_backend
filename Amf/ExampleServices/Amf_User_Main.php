...
class Amf_User_Main {
	...
	public function insertinject($type,$money,$rid,$uid)
	{
		...
		$stmt = $pdo->prepare($tsql); $stmt->execute();
		$psql="update user set currentmoney=currentmoney-$money where id=$uid";
		$stmt = $pdo->prepare($psql); $stmt->execute();
	}
	...
	{ 
		...
		$stmt = $pdo->prepare($tsql); $stmt->execute();$stmt=null;$pdo=null;
	}
	...
	{ 
		...
		if($result2)
		{
			...
			if($data->telMin<100 || $data->telMin>10000)
				$data->telMin=100;			
	...
	public function gethistoryBootXipai($id)
	{
		$pdo = MySQLUtil::getConnection();
		$today = date("Y-m-d",time())." 00:00:00";
		$tsql="select gameBoot,gameState from `round` where tab_id=".intval($id)." and createtime>'$today' order by rid desc";
		$stmt = $pdo->prepare($tsql); $stmt->execute(); $result2 = $stmt->fetch(PDO::FETCH_OBJ);$stmt=null;$pdo=null;
	...
	{   
		...
		if($boot!="6000")
		{
			$tsql="select gameid,gameType,gameTableName,result,gameState from `round`,tablet where `round`.gameState in (0,3) and tablet.tab_id=`round`.tab_id and tablet.gameid=".$id." and `round`.gameBoot=".$boot." and createtime>'$today' and `round`.result>=0 order by rid asc";
			$stmt = $pdo->prepare($tsql); $stmt->execute(); $result2 = $stmt->fetchAll(PDO::FETCH_OBJ);$stmt=null;$pdo=null;
			if(count($result2)>0)
			{
	...
	{ 
		...
		if($id=="01" || $id=="06" || $id=="08" || $id=="21")
			...
			$data->customChipArr=array(100,1000,5000,10000,50000,100000); 
		else if($id=="22" || $id=="25")
			$data->customChipArr=array(100,500,1000,5000,10000,20000); 
		else
	...
	public function groupInjectMoney($tid,$allseat)
	{
		...
		if($len>0)
		{
			...
			{ 
				...
				$stmt = $pdo->prepare($tsql); $stmt->execute(); $result = $stmt->fetch(PDO::FETCH_OBJ); 
				if($result)
				{
				...
				$stmt = $pdo->prepare($tsql); $stmt->execute(); $result3 = $stmt->fetchAll(PDO::FETCH_OBJ);
				if($result3)
				{
		...
		$res->type="groupInjectMoney";  $stmt=null;$pdo=null;
	...
	public function gameResults($tabid,$s,$n)
	{ 
		// $boot = self::gethistoryBoot($id);
		$pdo = MySQLUtil::getConnection();
