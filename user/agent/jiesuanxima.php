...
{
	$pid = $_POST['pid'];
	$puser = $DB->Select("select * from user where id = $pid");
	$puser = $puser[0];
	$ppid = $puser['pid'];
	$auser = $DB->Select("select * from user where id = $ppid");
	$auser = $auser[0]; 
	...
	if($lmoney!="0")
	{
		$DB->Query("update user set currentxima=currentxima-$lmoney where id = $pid");		
	}
	
	echo "<script>alert('结算成功！');window.location='/user/agent/ximareport.php?key=$uname';</script>";exit();
}
...
<table cellpadding="0" cellspacing="1" class="table_form">
	...
	<tr>
      ...
      <tr>
        ...
        <td>
<input boxid="auto" onkeyup="pay()" name="liveMoney" type="text" id="liveMoney" value="0" size="12" maxlength="12" require="true" datatype="double|range" min="0" max="<?=$user['currentxima']?>" msg="洗码量限额最小为0|洗码量限额最大为<?=$user['currentxima']?>" msgid="liveMoney1"/>
