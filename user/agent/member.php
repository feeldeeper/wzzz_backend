...
$member = $database->query("select * from user where pid = $id and type=3")->fetchAll();
$suser = $database->query("select * from user where id = $uid")->fetch();
...
<table cellpadding="0" cellspacing="1" class="table_list">
    <caption>
        <?= $user['username'] ?> <?php if ($user['type'] == 2) {
            echo "总代理";
        } elseif ($user['type'] == 3) {
            echo "代理";
        } else {
    ...
    <tr>
        <td colspan="4">扩展功能：<span class="align_c">
          <input class="button_style" onclick="redirect('/user/agent/add.php?type=member&id=<?= $user['id'] ?>')"
                 type='button' value='添加下级代理'/>
		  <input class="button_style" type='button' value='添加会员'
                 onclick="redirect('/user/agent/add.php?type=member&id=<?= $user['id'] ?>')"/>
		  <input class="button_style" type='button' value='<?= $user['username'] ?> 的下级'
                 onclick="redirect('/user/agent/index.php?id=<?= $user['id'] ?>')"/>
		  <input class="button_style" type='button' value='加减点'
                 onclick="redirect('/user/agent/editmoney.php?id=<?= $user['id'] ?>')"/>
		  <input class="button_style" type='button' value='日志'
                 onclick="redirect('/user/log/index.php?s_username=<?= $user['username'] ?>')"/>
		  <input class="button_style" type='button' value='权限'
                 onclick="redirect('/user/agent/acledit.php?id=<?= $user['id'] ?>')"/>
		  <input class="button_style" type='button' value='子账号'
                 onclick="redirect('/user/agent/submanage.php?id=<?= $user['id'] ?>')"/>
        </span></td>
    </tr>
</table>
    ...
    <form method="post" name="myform">
        ...
        <?php foreach ($member as $mb) { ?>
            <tr>
                ...
                <td><a href="#"
                       title="点击查看下级用户::最后登入时间：<?= $mb['lastlogintime'] ?><br>最后登入IP：<?= $mb['ip'] ?> - <?php $ip = $mb['ip'];
                       echo $database->GetIpLocData($ip); ?><br>登入次数：<?= $mb['logintimes'] ?>"><?= $mb['username'] ?></a></td>
                <td><?= $mb['nickName'] ?></td>
...
<div id="pages">
    <div id="pages">总数：<b><?php echo count($member); ?></b>&nbsp;
        每页：<b>20</b>
        <a href="/user/agent/member.php?id=<?= $user['id'] ?>&page=1">首页</a><a
            href="/user/agent/member.php?id=<?= $user['id'] ?>&page=1">上一页</a><a
            href="/user/agent/member.php?id=<?= $user['id'] ?>&page=1">下一页</a><a
            href="/user/agent/member.php?id=<?= $user['id'] ?>&page=1">尾页</a>
        页次：<b><font color="red">1</font>/1</b>
        <input type="text" name="page" id="page" size="4"
               onKeyDown="if(event.keyCode==13) {redirect('/user/agent/member.php?id=<?= $user['id'] ?>&page='+this.value); return false;}">
        <input type="button" value="转到" class="button_style"
               onclick="redirect('/user/agent/member.php?id=<?= $user['id'] ?>&page='+$('#page').val())"></div>
</div>
</form>
