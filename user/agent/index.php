...
$user = $user[0];
$agent = $DB->Select("select * from user where pid = $uid and type =2");
...
<table cellpadding="0" cellspacing="1" class="table_list">
  <caption>
  <?=$user['username']?> <?php if($user['type']=="1") echo "总";echo "代理";?> 信息  </caption>
    <tr>
    ...
    <tr>
    	<td colspan="4">扩展功能：<span class="align_c">
          <input class="button_style" onclick="redirect('/user/agent/add.php?type=agent&id=<?=$uid?>')" type='button' value='添加下级 代理' />
		  <input class="button_style" type='button' value='添加会员' onclick="redirect('/user/agent/add.php?type=member&id=<?=$uid?>')"/>
		  <input class="button_style" type='button' value='<?=$user['username']?> 的会员' onclick="redirect('/user/agent/member.php?id=<?=$uid?>')"/>
		  <input class="button_style" type='button' value='日志' onclick="redirect('/user/log/index.php?s_username=<?=$username?>')"/>
		  <input class="button_style" type='button' value='子账号' onclick="redirect('/user/agent/submanage.php?id=<?=$uid?>')"/>
		  <?php if($user['type']!="0"){ ?>
		  <input class="button_style" type='button' value='洗码佣金报表' onclick="redirect('/user/agent/ximareport.php')"/>
		  <?php } ?>
        </span></td>
    </tr>
</table>
...
<form method="post" name="myform">
  <caption>
       <?=$user['username']?> 下线 代理 账号列表  </caption>
    <tr align="center">
...
<tr>
        ...
        <td><a href="/user/agent/index.php?id=<?=$ag['id']?>" title="点击查看下级用户::最后登入时间：<?=$ag['lastlogintime']?><br>最后登入IP：<?=$ag['ip']?> - <?php $ip=$ag['ip'];echo $DB->GetIpLocData($ip);?><br>登入次数：<?=$ag['logintimes']?>"><?=$ag['username']?></a></td>
        <td><?=$ag['nickName']?></td>
        <td>人民币</td>
...
每页：<b>20</b>
<a href="/user/agent/index.php?id=<?=$uid?>&page=1">首页</a><a href="/user/agent/index.php?id=<?=$uid?>&page=1">上一页</a><a href="/user/agent/index.php?id=<?=$uid?>&page=1">下一页</a><a href="/user/agent/index.php?id=<?=$uid?>&page=1">尾页</a>
页次：<b><font color="red">1</font>/1</b>
<input type="text" name="page" id="page" size="4" onKeyDown="if(event.keyCode==13) {redirect('/user/agent/index.php?id=<?=$uid?>&page='+this.value); return false;}"> 
<input type="button" value="转到" class="button_style" onclick="redirect('/user/agent/index.php?id=<?=$uid?>&page='+$('#page').val())"></div></div>
</form>
