...
if (isset($_POST['dosubmit']) && $_POST['dosubmit'] != "") {
    ...
    if ($ptype != "0" && $lmoney != "0") {
        $database->query("update user set currentmoney=currentmoney-$lmoney where id = $pid");
    }
...
<form name="myform" method="post" action="add.php?type=<?= $type ?>&id=<?= $id ?>">
    ...
    <table cellpadding="0" cellspacing="1" class="table_form">
        ...
        <tr>
            ...
            <td>
                <?php if ($user['type'] == "1") echo "总";
                echo "代理：" . $user['username']; ?>      </td>
        </tr>
        <tr>
            <th width="20%"><strong>用户名：</strong></th>
            <td width="80%">
                <input type="text" style="ime-mode:disabled" name="userName" id="userName" size="16" class=""
                       maxlength="20" require="true" datatype="limit|ajax" url="/user/agent/checkuser.php" min="2"
                       max="20" msg="用户名不得少于2个字符超过20个字符|"/></td>
        </tr>
        <tr>
    ...
    <input type="text" name="tel" id="tel" size="16" maxlength="16" value="">        </td>
        ...
        <tr>
            ...
            <td>
                <input boxid="auto" name="liveMoney" type="text" id="liveMoney" value="0" size="12" maxlength="12"
                       require="true" datatype="double|range" min="0"
                       max="<?php if ($muser['type'] == '0') echo "10000000000";
                       else echo $user['currentmoney']; ?>"
                       msg="真人点数限额最小为0|<?php if ($muser['type'] == '0') echo "真人点数无上限";
                       else echo "真人点数限额最大为" . $user['currentmoney']; ?>" msgid="liveMoney1"/>
                &nbsp;<font
                    style="color:red"> <?php if ($muser['type'] != '0') { ?>/ <?= $user['currentmoney'] ?><?php } ?></font><span
                    id="liveMoney1"></span>
        ...
        <tr>
            ...
            <td>
                <?php if ($user['type'] == "1") { ?>
                    <input type="radio"
                           onclick="$('#liveWash2').html('1.8');$('#liveWash').val('1.8');$('#liveWash').attr('max','1.8');"
                           name="ximatype" checked value="1">单边
                    <input type="radio"
                           onclick="$('#liveWash2').html('0.9');$('#liveWash').val('0.9');$('#liveWash').attr('max','0.9');"
                           name="ximatype" value="0">双边
                <?php } elseif ($user['type'] == "2") { ?>
                    <?php if ($user['danshuangbian'] == "1") { ?><input type="radio" name="ximatype" checked
                                                                        value="1">单边<?php } ?>
                    <?php if ($user['danshuangbian'] == "0") { ?><input type="radio" name="ximatype" checked
                                                                        value="0">双边<?php } ?>
                <?php } ?>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <input boxid="auto" name="liveWash" id="liveWash" type="text" value="1.8" size="5" maxlength="5"
                       require="true" datatype="double|range" min="0" max="<?= $ximalimit ?>"
                       msg="洗码比最小为0%|洗码比最大为双边0.9%，单边1.8%" msgid="liveWash1"/>
            ...
            <td>
                <div style="width:350px">
                    <table cellpadding="0" cellspacing="1" class="table_list align_c" id="gamesP">
                        ...
                        <tr>
                            <th>百家乐</th>
                            <td><input type="checkbox" boxid="gamesLoginAll" checked name="gamesLogin[]" value="0"/>
                            </td>
                            <td><input type="checkbox" checked boxid="gamesPlayGameAll" name="gamesPlayGame[]"
                                       value="0"/></td>
                        </tr>
                        <tr>
                            <th>龙虎</th>
                            <td><input type="checkbox" checked boxid="gamesLoginAll" name="gamesLogin[]" value="1"/>
                            </td>
                            <td><input type="checkbox" checked boxid="gamesPlayGameAll" name="gamesPlayGame[]"
                                       value="1"/></td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td><input type="checkbox" id="gamesPlayGameAll" onclick="checkall('gamesPlayGameAll')"/>
                            </td>
                        </tr>
                    </table>
                </div>
