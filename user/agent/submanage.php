  ...
  <?=$user['username']?>代理信息
    ...
    <tr>
    	<td colspan="4">扩展功能：<span class="align_c">
          <input class="button_style" onclick="redirect('/user/agent/submanageadd.php?id=<?=$user['id']?>')" type='button' value='添加子账号' />
    	</span></td>
    </tr>
</table>
...
<form method="post" name="myform">
  <caption>
    ...
    <tr align="center">
        ...
        <th width="10%">
        	<strong>
            	<a href="http://admin.usemm.com/phpcms/admin.php?mod=member&file=member&action=manage&orderby=m.username ASC" title="按用户名排序">用户名</a>            </strong>        </th>
...
每页：<b>20</b>
<a href="/user/agent/submanage.php?id=<?=$user['id']?>&page=1">首页</a><a href="/user/agent/submanage.php?id=<?=$user['id']?>&page=1">上一页</a><a href="/user/agent/submanage.php?id=<?=$user['id']?>&page=1">下一页</a><a href="/user/agent/submanage.php?id=<?=$user['id']?>&page=1">尾页</a>
页次：<b><font color="red">1</font>/0</b>
<input type="text" name="page" id="page" size="4" onKeyDown="if(event.keyCode==13) {redirect('/user/agent/submanage.php?id=<?=$user['id']?>&page='+this.value); return false;}"> 
<input type="button" value="转到" class="button_style" onclick="redirect('/user/agent/submanage.php?id=<?=$user['id']?>&page='+$('#page').val())"></div></div>
</form>
