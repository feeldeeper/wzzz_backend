...
$pid = " ('$username')";
if ($key == "") {
    $pid = getpid($username, $database);
}

$logs = ximalog($key, $pid, $stime, $etime, $database);

function ximalog($key, $pid, $stime, $etime, $database)
{
    $arr = array();
...
function getpid($username, $database)
{
    $id = getid($username, $database);
