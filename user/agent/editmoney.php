...
if (isset($_POST['uid']) && $_POST['uid'] != "") {
    ...
    if ($type == "1") {
        ...
        if ($ptype != "0") {
            $database->query("update user set currentmoney = currentmoney - $im where id = $pid");
        }
    } else {
        ...
        if ($ptype != "0") {
            $database->query("update user set currentmoney = currentmoney + $om where id = $pid");
        }
    }
...
if (isset($_GET['id']) && $_GET['id'] != "") {
    ...
    $suser = $database->query("select * from user where id = $suid")->fetch();
} else {
    ...
    <table cellpadding="0" cellspacing="1" class="table_form">
        ...
        <tr>
            ...
            <td width="75%">
                ...
                <input type="radio" onclick="selectPayType(1)" value="0" name="type" class="radio_style"/>&nbsp;减点(-)&nbsp;
            </td>
        </tr>
        <tr>
            <th><strong>类型</strong></th>
            <td>
                <input name="moneyType" type="radio" class="radio_style" onclick="javascript:change();" value="0"
                       checked="checked"/>真人点数&nbsp;&nbsp;    </td>
        </tr>
        ...
        <tr id="payIn">
            ...
            <td>
                <input type="text" onclick="if(this.value == 0){this.value = ''}" id="payInMoney" onkeyup="pay()"
                       name="payInMoney" value="0" size="12" require="true" datatype="currency|range" min="0"
                       max="<?php echo floor($suser['currentmoney']); ?>"
                       msg="真人点数限额最小为0|真人点数限额最大为<?php if ($suser['type'] == "0") echo "无限"; else echo $suser['currentmoney']; ?>"
                       msgid="code1"/>
                &nbsp;<font id="numberid" name="numberid" style="color:red">点</font><span
                    id="code1"></span><?php if ($suser['type'] != "0") { ?>，上级<b
                    class="r"><?= $suser['username'] ?></b>还有<b class="r" id="balance1"></b>可加<b class="r"
                                                                                                 id="balance2"><?= $suser['currentmoney'] ?></b><?php } ?>
            </td>
        </tr>
        ...
        <tr id="payOut" style="display:none">
            ...
            <td>
                <input type="text" id="payOutMoney" onkeyup="pay()" name="payOutMoney" value="0" size="12"
                       require="true" datatype="currency|range" min="0" max="<?= $user['currentmoney'] ?>"
                       msg="真人点数限额最小为0|真人点数限额最大为<?= $user['currentmoney'] ?>" msgid="code2"/>
                &nbsp;<font id="numberid" name="numberid" style="color:red">点</font><span id="code2"></span>，下级<b
                    class="r"><?= $user['username'] ?></b>还有<b class="r" id="balance3"></b>可减<b class="r"
                                                                                                id="balance4"></b>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td><textarea name="note" cols="60" rows="8" id="kw"></textarea><span id="su"></span>&nbsp;<font
