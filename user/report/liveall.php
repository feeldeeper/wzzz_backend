...
$key = $username;if(isset($_GET['key'])){$key=$_GET['key'];
}
...
$ssuid = $uid;
$uid = getid($key,$conn);
if(!$DB->VerifyUserReport($admintab,$ssuid,$uid))
{
	echo "没有权限！";exit();
}
$mems = allmem($uid,"3",$conn);
$ages = allmem($uid,"2",$conn);

foreach($mems as $m)
{
	$member=memberresult($m,$stime,$etime,$conn);
...
foreach($ages as $m)
{	
	$agent=agentresult($m,$stime,$etime,$conn);
	array_push($agents,$agent);
	$agent=null;
}

function allmem($id,$type,$conn)
{
	$ags = array();
...
function agentresult($id,$stime,$etime,$conn)
{
	...
	$member->shijimoney=shijimoney($id,$stime,$etime,$conn);
	
	return $member;
}

function shijimoney($id,$stime,$etime,$conn)
{
...
function memberresult($id,$stime,$etime,$conn)
{
	$query = "select * from user where id=$id";	
	$result=mysql_query($query,$conn);
	if(mysql_num_rows($result))
...
<tr align="center">
  <td><input type='button' class="button_style" onclick="redirect('/user/report/livebjl.php?username=<?php echo $ag->username;?>&stime=<?=$stime?>&etime=<?=$etime?>');" value='明细'></td>
  <td></td>
...
<tbody>
<?php $key=0; foreach($members as $mem){$key++;?>
<tr>
	<td><input type='button' class="button_style" onclick="redirect('/user/report/livebjl.php?username=<?php echo $ag->username;?>&stime=<?=$stime?>&etime=<?=$etime?>');" value='明细'></td>
	<td><?=$key?></td>
...
0.07221389<object id="LODOP_OB" classid="clsid:2105C259-1E0C-4534-8141-A753534CB4CA" width=0 height=0> 
	<embed id="LODOP_EM" type="application/x-print-lodop" width=0 height=0 pluginspage="/js/install_lodop32.exe"></embed>
</object>
<div style="text-align:center; width:660px; margin:0; padding:0; display:none;position:relative"" id="userPrintTemplates">
...
<h4 style="margin:4px;text-align:center"><span id="pStime"></span>&nbsp;&nbsp;至&nbsp;&nbsp;<span id="pEtime"></span></h4>
<table width="100%" border="1" cellpadding="4" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;border:solid 1px">
  <tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin:6px 0 0 0">
  <tr>
    ...
    <td>主管审批：</td>
    <td>领款人：</td>
    <td width="160" align="right"><span id="thisYears"></span>年&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;月&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;日</td>
  </tr>
</table>
</div>
...
<h4 style="margin:4px;text-align:center"><span id="apStime"></span>&nbsp;&nbsp;至&nbsp;&nbsp;<span id="apEtime"></span></h4>
<table width="100%" border="1" cellpadding="4" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;border:solid 1px">
  <tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin:6px 0 0 0">
  <tr>
    ...
    <td>主管审批：</td>
    <td>领款人：</td>
    <td width="160" align="right"><span id="apthisYears"></span>年&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;月&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;日</td>
  </tr>
</table>
</div>
<script>
