...
$user=$DB->Query("select * from user where username='$key'");
$user = $user[0];
...
<input type='button' class="button_style" id="fullScreen" value='全屏显示'>
    ...
    <tr>
    	<td>
按用户名查询：
  <input name="userName" type="text" id="userName" onKeyDown="if(event.keyCode==13) {go(0);}" value="<?=$key?>" size="20"/>
...
<tr align="center">
  <td><input type='button' class="button_style" onclick="redirect('/user/report/bjlmember.php?id=<?php echo $mem->id;?>&stime=<?=$stime?>&etime=<?=$etime?>');" value='明细'></td>
...
<tbody>
<?php $key=0; foreach($agents as $ag){ $key++;?>
