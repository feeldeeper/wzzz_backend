...
$notice = $database->query("select * from notice")->fetchAll();
...
<table cellpadding="0" cellspacing="1" class="table_list align_c">
    ...
    <?php foreach ($notice as $no) { ?>
        <tr>
            ...
            <td><?= $no['stime'] ?></td>
            <td><?= $no['etime'] ?></td>
            <td><?php if ($admintab == "1") { ?><a href="/user/log/edit.php?id=<?= $no['id'] ?>">修改</a><?php } ?></td>
        </tr>
    <?php } ?>
</table>
<div id="pages">
    <div id="pages">总数：<b><?= count($notice) ?></b>&nbsp;
        每页：<b>20</b>
        <a href="notice.php?page=1">首页</a><a href="notice.php?page=1">上一页</a><a href="notice.php?page=1">下一页</a><a
            href="notice.php?page=1">尾页</a>
        页次：<b><font color="red">1</font>/1</b>
        <input type="text" name="page" id="page" size="4"
               onKeyDown="if(event.keyCode==13) {redirect('/user/log/notice.php?page='+this.value); return false;}">
        <input type="button" value="转到" class="button_style"
               onclick="redirect('/user/log/notice.php?page='+$('#page').val())"></div>
</div>
</body>
</html>