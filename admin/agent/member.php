...
<DIV id=position2><STRONG>当前位置：</STRONG>
    <A href='/admin/agent/index.php'><?= $user['username'] ?></A></DIV>
    ...
    <tr>
        <td colspan="4">扩展功能：<span class="align_c">
		  ...
		  <input class="button_style" type='button' value='<?= $user['username'] ?> 的下级'
                 onclick="redirect('/admin/agent/index.php?id=<?= $user['id'] ?>')"/>
		  <input class="button_style" type='button' value='加减点'
                 onclick="redirect('/admin/agent/editmoney.php?id=<?= $user['id'] ?>')"/>
        ...
        <?php foreach ($member as $mb) { ?>
            <tr>
                ...
                <td><a href="#"
                       ...
                       echo $database->GetIpLocData($ip); ?><br>登入次数：<?= $mb['logintimes'] ?>"><?= $mb['username'] ?></a>
                </td>
                <td><?= $mb['nickName'] ?></td>
                ...
                <td>
                    <a href="/admin/agent/edit.php?id=<?= $mb['id'] ?>">修改</a>
                    |
                    <a href="/admin/agent/del.php?id=<?= $mb['id'] ?>" onclick="return confirm('确定要删除吗?')">删除</a>
                    |
                    <a href="/admin/agent/editmoney.php?id=<?= $mb['id'] ?>">加减点</a>
                    <!--|
                    <a href="#">上线提醒</a>
                    |
                    <a href="#">限制登入地区</a>-->
...
<div id="pages">
    <div id="pages">总数：<b><?php echo count($member); ?></b>&nbsp;
        每页：<b>20</b>
        <a href="/admin/agent/member.php?id=<?= $user['id'] ?>&page=1">首页</a><a
            href="/admin/agent/member.php?id=<?= $user['id'] ?>&page=1">上一页</a><a
            href="/admin/agent/member.php?id=<?= $user['id'] ?>&page=1">下一页</a><a
            href="/admin/agent/member.php?id=<?= $user['id'] ?>&page=1">尾页</a>
        页次：<b><font color="red">1</font>/1</b>
        <input type="text" name="page" id="page" size="4"
               onKeyDown="if(event.keyCode==13) {redirect('/admin/agent/member.php?id=<?= $user['id'] ?>&page='+this.value); return false;}">
        <input type="button" value="转到" class="button_style"
               onclick="redirect('/admin/agent/member.php?id=<?= $user['id'] ?>&page='+$('#page').val())"></div>
</div>
</form>
