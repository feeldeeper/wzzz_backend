  ...
  <?=$user['username']?> <?php if($user['type']==2){echo "总代理";}elseif($user['type']==3){echo "代理";}else{echo "管理员";}?> 信息  </caption>
    ...
    <tr>
    	<td colspan="4">扩展功能：<span class="align_c">
		  ...
		  <input class="button_style" type='button' value='<?=$user['username']?> 的下级' onclick="redirect('/admin/agent/index.php?id=<?=$user['id']?>')"/>
		  <input class="button_style" type='button' value='加减点' onclick="redirect('/admin/agent/editmoney.php?id=<?=$user['id']?>')"/>
...
每页：<b>20</b>
<a href="/admin/agent/member.php?id=<?=$user['id']?>&page=1">首页</a><a href="/admin/agent/member.php?id=<?=$user['id']?>&page=1">上一页</a><a href="/admin/agent/member.php?id=<?=$user['id']?>&page=1">下一页</a><a href="/admin/agent/member.php?id=<?=$user['id']?>&page=1">尾页</a>
页次：<b><font color="red">1</font>/1</b>
<input type="text" name="page" id="page" size="4" onKeyDown="if(event.keyCode==13) {redirect('/admin/agent/member.php?id=<?=$user['id']?>&page='+this.value); return false;}"> 
<input type="button" value="转到" class="button_style" onclick="redirect('/admin/agent/member.php?id=<?=$user['id']?>&page='+$('#page').val())"></div></div>
</form>
