...
$user = $user[0];
$agent = $database->query("select * from user where pid = $uid and type =2");
...
function lock(id, type, obj){
	...
	if (confirm("确认要"+str+"【" +userName+ "】的登录权限吗？")) {
		redirect('/admin/manage/lock/id/' + id + '/type/' + type + '/username/' + userName);
	}
}
function liveLock(id, type, obj){
	...
	if (confirm("确认要"+str+"【" +userName+ "】的投注权限吗？")) {
		redirect('/admin/manage/lock/model/1/id/' + id + '/type/' + type + '/username/' + userName);
	}
}
function hideLock(id, type, obj){
	redirect('/admin/manage/hide/model/1/id/' + id + '/type/' + type + '/username/' + $(obj).parents("tr").find("td").eq(1).text());
}
  ...
  <?=$user['username']?> <?php if($user['type']=="1") echo "总";echo "代理";?> 信息  </caption>
    ...
    <tr>
    	<td colspan="4">扩展功能：<span class="align_c">
		  ...
		  <input class="button_style" type='button' value='<?=$user['username']?> 的会员' onclick="redirect('/admin/agent/member.php?id=<?=$uid?>')"/>
		  <input class="button_style" type='button' value='加减点' onclick="redirect('/admin/agent/editmoney.php?id=<?=$uid?>')"/>
...
<tr>
        ...
        <td><a href="/admin/agent/index.php?id=<?=$ag['id']?>" title="点击查看下级用户::最后登入时间：<?=$ag['lastlogintime']?><br>最后登入IP：<?=$ag['ip']?> - <?php $ip=$ag['ip'];echo $DB->GetIpLocData($ip);?><br>登入次数：<?=$ag['logintimes']?>"><?=$ag['username']?></a></td>
        <td><?=$ag['nickName']?></td>
        <td>人民币</td>
...
<input name="button2" type="button" class="button_style"  title="点击锁定该账号投注权限" value="开启" />		</td>
        ...
        <td>
<a href="/admin/agent/edit.php?id=<?=$ag['id']?>">修改</a>
| 
<a href="/admin/agent/del.php?id=<?=$ag['id']?>" onclick="return confirm('确定要删除吗?')">删除</a>
|
<a href="/admin/agent/editmoney.php?id=<?=$ag['id']?>">加减点</a>
</td>
  </tr>
 <?php } ?>
...
每页：<b>20</b>
<a href="/admin/agent/index.php?id=<?=$uid?>&page=1">首页</a><a href="/admin/agent/index.php?id=<?=$uid?>&page=1">上一页</a><a href="/admin/agent/index.php?id=<?=$uid?>&page=1">下一页</a><a href="/admin/agent/index.php?id=<?=$uid?>&page=1">尾页</a>
页次：<b><font color="red">1</font>/1</b>
<input type="text" name="page" id="page" size="4" onKeyDown="if(event.keyCode==13) {redirect('/admin/agent/index.php?id=<?=$uid?>&page='+this.value); return false;}"> 
<input type="button" value="转到" class="button_style" onclick="redirect('/admin/agent/index.php?id=<?=$uid?>&page='+$('#page').val())"></div></div>
</form>
