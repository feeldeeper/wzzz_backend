    ...
    <tr>
        <td colspan="4">扩展功能：<span class="align_c">
		  ...
		  <input class="button_style" type='button' value='<?= $user['username'] ?> 的会员'
                 onclick="redirect('/admin/agent/member.php?id=<?= $uid ?>')"/>
		  <input class="button_style" type='button' value='加减点'
                 onclick="redirect('/admin/agent/editmoney.php?id=<?= $uid ?>')"/>
    ...
    <form method="post" name="myform">
        ...
        <?php foreach ($agent as $ag) { ?>
            <tr>
                ...
                <td><a href="/admin/agent/index.php?id=<?= $ag['id'] ?>"
...
<div id="pages">
    <div id="pages">总数：<b><?php echo count($agent); ?></b>&nbsp;
        每页：<b>20</b>
        <a href="/admin/agent/index.php?id=<?= $uid ?>&page=1">首页</a><a
            href="/admin/agent/index.php?id=<?= $uid ?>&page=1">上一页</a><a
            href="/admin/agent/index.php?id=<?= $uid ?>&page=1">下一页</a><a
            href="/admin/agent/index.php?id=<?= $uid ?>&page=1">尾页</a>
        页次：<b><font color="red">1</font>/1</b>
        <input type="text" name="page" id="page" size="4"
               onKeyDown="if(event.keyCode==13) {redirect('/admin/agent/index.php?id=<?= $uid ?>&page='+this.value); return false;}">
        <input type="button" value="转到" class="button_style"
               onclick="redirect('/admin/agent/index.php?id=<?= $uid ?>&page='+$('#page').val())"></div>
</div>
</form>
